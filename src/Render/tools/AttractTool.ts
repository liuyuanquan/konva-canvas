...
export class AttractTool extends BaseTool {
	...
	getSortItems(rect: AlignRect) {
		...
		sortY.push(
			...
			{
				value: this.render.toStageValue(
					rect.y - stageState.y + rect.height / 2
				), // 水平中
			},
			{
		...
		for (const node of otherNodes) {
			...
			sortX.push(
				...
				},
				{
					id: node._id,
					value: this.render.toStageValue(
						nodeRect.x - stageState.x + nodeRect.width / 2
					), // 垂直中
				},
				{
					id: node._id,
					value: this.render.toStageValue(
						nodeRect.x - stageState.x + nodeRect.width
					), // 右
				}
			);
			// y
			sortY.push(
				...
				},
				{
					id: node._id,
					value: this.render.toStageValue(
						nodeRect.y - stageState.y + nodeRect.height / 2
					), // 水平中
				},
				{
					id: node._id,
					value: this.render.toStageValue(
						nodeRect.y - stageState.y + nodeRect.height
					), // 下
				}
			);
		}
	...
	attractPoint = (pos: Konva.Vector2d) => {
		...
		if (!this.render.config.readonly && this.render.config.attractNode) {
			...
			for (let i = 0; i < sortX.length - 1; i++) {
				const current = sortX[i];
				const next = sortX[i + 1];
				if (!current || !next) continue;

				// 相邻两个点，必须为 目标节点 + 非目标节点
				if (
					(current.id === void 0 && next.id !== void 0) ||
					(current.id !== void 0 && next.id === void 0)
				) {
					// 相邻两个点的 x 距离
					const offset = Math.abs(current.value - next.value);
					if (offset < XMin) {
						// 更新 x 最短距离 记录
						XMin = offset;
						// 更新 x 最短距离的【对】 记录
						pairXMin = [[current, next]];
					} else if (offset === XMin) {
						// 存在多个 x 最短距离
						pairXMin.push([current, next]);
					}
				}
			}

			for (let i = 0; i < sortY.length - 1; i++) {
				const current = sortY[i];
				const next = sortY[i + 1];
				if (!current || !next) continue;
				...
				if (
					...
					if (offset < YMin) {
						...
						YMin = offset;
						// 更新 y 最短距离的【对】 记录
						pairYMin = [[current, next]];
					} else if (offset === YMin) {
						// 存在多个 y 最短距离
						pairYMin.push([current, next]);
					}
				}
			}
			...
			if (pairXMin[0] && pairXMin[0][0] && pairXMin[0][1]) {
				if (
			...
			if (pairYMin[0] && pairYMin[0][0] && pairYMin[0][1]) {
				if (
				...
				for (const diff of [
					...
					if (diff.value < 5) {
						if (diff.type === "stageRightX") {
							newPosX =
								this.render.toBoardValue(logicStageRightX) + stageState.x;
						} else if (diff.type === "leftX") {
							newPosX =
								this.render.toBoardValue(logicClosestLeftX) + stageState.x;
						} else if (diff.type === "rightX") {
							newPosX =
								this.render.toBoardValue(logicClosestRightX) + stageState.x;
						}
						isAttract = true;
						break;
					}
				...
				for (const diff of [
					...
					if (diff.value < 5) {
						if (diff.type === "stageBottomY") {
							newPosY =
								this.render.toBoardValue(logicStageBottomY) + stageState.y;
						} else if (diff.type === "topY") {
							newPosY =
								this.render.toBoardValue(logicClosestTopY) + stageState.y;
						} else if (diff.type === "bottomY") {
							newPosY =
								this.render.toBoardValue(logicClosestBottomY) + stageState.y;
						}
						isAttract = true;
						break;
					}
