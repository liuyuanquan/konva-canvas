...
export class CopyTool extends BaseTool {
	...
	nodesIdCover(nodes: Konva.Node[]) {
		...
		for (const node of nodes) {
			if (node instanceof Konva.Group) {
				...
				node.on("mouseenter", () => {
					// 显示 连接点
					// this.render.linkTool.pointsVisible(true, node) // LinkTool 已删除
				});
				node.off("mouseleave");
				node.on("mouseleave", () => {
					// 隐藏 连接点
					// this.render.linkTool.pointsVisible(false, node) // LinkTool 已删除
				...
				node.setAttrs({
					x:
						node.x() +
						this.render.toStageValue(this.render.bgSize) * this.pasteCount,
					y:
						node.y() +
						this.render.toStageValue(this.render.bgSize) * this.pasteCount,
				});
				...
				if (node.attrs.manualPointsMap) {
					...
					for (const pairId in node.attrs.manualPointsMap) {
						manualPointsMap[idMap.get(pairId)] =
							node.attrs.manualPointsMap[pairId];
					}
					...
					for (const pairId in manualPointsMap) {
						...
						if (Array.isArray(manualPoints)) {
							manualPointsMap[pairId] = manualPoints.map((o) => ({
								x:
									o.x +
									this.render.toStageValue(this.render.bgSize) *
										this.pasteCount,
								y:
									o.y +
									this.render.toStageValue(this.render.bgSize) *
	...
	copy(nodes: Konva.Node[]) {
		...
		// 重绘
		this.redraw(this.getCommonDrawNames());
	}
}
Object.defineProperty(CopyTool, "name", {
	value: "CopyTool",
	writable: false,
	configurable: false,
});
