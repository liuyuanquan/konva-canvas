...
export class AssetTool extends BaseTool {
	// 加载 svg xml
	...
	async loadSvg(src: string) {
		...
		return this.loadSvgXML(
			this.render.setSvgXMLSettings(svgXML, this.render.getAssetSettings())
		);
	}
	...
	async loadJson(src: string) {
		try {
			...
			// 重新加载 stage
			const stageReload = Konva.Node.create(
...
Object.defineProperty(AssetTool, "name", {
	value: "AssetTool",
	writable: false,
	configurable: false,
});
