	...
	handlers = {
		dom: {
			...
			drop: (e: GlobalEventHandlersEventMap["drop"]) => {
				...
				const morePointsTxt = e.dataTransfer?.getData("points")?.trim() || "[]";
					...
					if (pos) {
						this.render.assetTool[
							...
							if (target instanceof Konva.Image) {
								...
								group = target;
								// this.render.linkTool.groupIdCover(group); // LinkTool 已删除
							}
							...
							group.on("mouseenter", () => {
								// 显示 连接点
								// this.render.linkTool.pointsVisible(true, group); // LinkTool 已删除
							});
							...
							group.on("mouseleave", () => {
								// 隐藏 连接点
								// this.render.linkTool.pointsVisible(false, group); // LinkTool 已删除
