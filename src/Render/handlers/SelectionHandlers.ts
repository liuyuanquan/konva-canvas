...
export class SelectionHandlers extends BaseHandler {
	...
	transformerStateReset() {
		...
		const points = groups.reduce((ps, group) => {
			return ps.concat(
				Array.isArray(group.getAttr("points")) ? group.getAttr("points") : []
			);
		}, [] as Types.LinkDrawPoint[]);

		const pairs = points.reduce((ps, point) => {
			return ps.concat(
				point.pairs ? point.pairs.filter((o) => !o.disabled) : []
			);
			...
			mousedown: (
				...
				if (!this.render.graphType) {
					...
					if (e.target === this.render.stage) {
						...
						// 隐藏连接点
						// this.render.linkTool.pointsVisible(false) // LinkTool 已删除
		...
		transformer: {
			...
			transform: () => {
				...
				if (back) {
					...
					const points = groups.reduce((ps, group) => {
						return ps.concat(
							Array.isArray(group.getAttr("points"))
								? group.getAttr("points")
								: []
						);
					}, [] as Types.LinkDrawPoint[]);
					...
					for (const pair of pairs) {
						...
						if (fromGroup && toGroup) {
							...
							if (
								...
								fromGroup.attrs.manualPointsMapBefore
							) {
								let manualPoints = fromGroup.attrs.manualPointsMap[pair.id];
								const manualPointsBefore =
									fromGroup.attrs.manualPointsMapBefore[pair.id];
								if (
									Array.isArray(manualPoints) &&
									Array.isArray(manualPointsBefore)
								) {
									manualPoints = manualPointsBefore.map(
										(o: Types.ManualPoint) => {
			...
			dragmove: () => {
				...
				const { pos: transformerPos, isAttract } =
					this.render.attractTool.attractTransformer(rect);
											...
											(o: Types.ManualPoint) =>
												({
													x:
														o.x +
														this.render.toStageValue(
															rect.x - this.transformerMousedownPos.x
														),
													y:
														o.y +
														this.render.toStageValue(
															rect.y - this.transformerMousedownPos.y
														),
												} as Types.ManualPoint)
									  );
				...
				// 重绘
				this.render.redraw([Draws.RulerDraw.name]);
			},
			dragend: () => {
				// 拖动结束
			...
			if (!this.render.config.readonly && this.render.config.attractResize) {
				...
				if (anchor && anchor !== "rotater") {
					...
					if (snappedX || snappedY) {
						...
						return {
							x: snappedX
								? this.render.toBoardValue(logicClosestX) + stageState.x
								: newPos.x,
							y: snappedY
								? this.render.toBoardValue(logicClosestY) + stageState.y
								: newPos.y,
						};
					}
				}
...
Object.defineProperty(SelectionHandlers, "name", {
	value: "Selection",
	writable: false,
	configurable: false,
});
