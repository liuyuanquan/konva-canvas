		...
		if (this.state.target) {
			...
			if (this.state.target === this.render.stage) {
				...
				menus.push({
					name: "自适应大小",
					action: () => {
						// this.render.positionTool.positionFit() // PositionTool 已删除
					},
				});
				menus.push({
					name: "恢复位置",
					action: () => {
						// this.render.positionTool.positionReset() // PositionTool 已删除
					},
				});
				menus.push({
					name: "恢复大小位置",
					action: () => {
						// this.render.positionTool.positionZoomReset() // PositionTool 已删除
					},
				});
				...
				if (this.state.target.name() === "link-line") {
					menus.push({
						...
						action: () => {
							// 删除 连接线
							// this.render.linkTool.remove(this.state.target as Konva.Line) // LinkTool 已删除
						},
					});
				} else {
					...
					menus.push({
						...
						action: () => {
							if (target) {
								// this.render.zIndexTool.up([target]) // ZIndexTool 已删除
							}
						},
					});
					menus.push({
						...
						action: () => {
							if (target) {
								// this.render.zIndexTool.down([target]) // ZIndexTool 已删除
							}
						},
					});
					menus.push({
						...
						action: () => {
							if (target) {
								// this.render.zIndexTool.top([target]) // ZIndexTool 已删除
							}
						},
					});
					menus.push({
						...
						action: () => {
							if (target) {
								// this.render.zIndexTool.bottom([target]) // ZIndexTool 已删除
							}
						},
					});
						...
						if (pos) {
							...
							if (shapes.length > 1) {
								...
								menus.push({
									...
									action: () => {
										// this.render.alignTool.align(Types.AlignType.垂直居中, selected) // AlignTool 已删除
									},
								});
								menus.push({
									name: "左对齐" + (selected ? "于目标" : ""),
									action: () => {
										// this.render.alignTool.align(Types.AlignType.左对齐, selected) // AlignTool 已删除
									},
								});
								menus.push({
									name: "右对齐" + (selected ? "于目标" : ""),
									action: () => {
										// this.render.alignTool.align(Types.AlignType.右对齐, selected) // AlignTool 已删除
									},
								});
								menus.push({
									name: "水平居中" + (selected ? "于目标" : ""),
									action: () => {
										// this.render.alignTool.align(Types.AlignType.水平居中, selected) // AlignTool 已删除
									},
								});
								menus.push({
									name: "上对齐" + (selected ? "于目标" : ""),
									action: () => {
										// this.render.alignTool.align(Types.AlignType.上对齐, selected) // AlignTool 已删除
									},
								});
								menus.push({
									name: "下对齐" + (selected ? "于目标" : ""),
									action: () => {
										// this.render.alignTool.align(Types.AlignType.下对齐, selected) // AlignTool 已删除
									},
								});
							}
						}
			...
			contextmenu: (
				...
				if (!e.evt.ctrlKey) {
					...
					if (pos && this.state.lastPos) {
						// TODO: LinkDraw 已删除，需要重新实现
						// const linkGroup = this.render.layerCover.find(
						//   `.${Draws.LinkDraw.name}`
						// )[0] as Konva.Group
						const linkGroup = undefined as Konva.Group | undefined;
